...
class DataSimulator:
    ...
    def simulate_data_by_rep(self, num_proteins,num_conditions, num_reps, num_features, mar_thresh=.05):
        ...
        for i in range(self.number_nodes):
            ...
            for j in range(num_conditions):
                if np.random.randint(0,1000) <= 25:
                    print("here")
                    condition_effect.append(np.random.uniform(overall_mean*.5, overall_mean*.9))
                elif np.random.randint(0,1000) <= 50:
                    print("here2")
                    condition_effect.append(np.random.uniform(-overall_mean*.4, -overall_mean*.6))
                else:
        ...
        for i in range(len(nodes)):
            overall_mean = sim_data.loc[sim_data["Protein"] == i]["Protein_mean"].unique()
            temp_vals = np.random.normal(0., overall_mean*.2, num_features*num_conditions)
...
def main():
    ...
    n=200
    simulator.simulate_data_by_rep(n, 4, 3, 3, mar_thresh=0.03)
    # simulator.simulate_data(5, 4, 2, 10, 4)
    for i in range(n):
        print(simulator.data[simulator.data["Protein"] == i]["Missing"].sum())

    ## Save whole obj to pickle
    with open(r"../data/simulated_data_200.pickle", "wb") as output_file:
        pickle.dump(simulator, output_file)

    ## Save data to csv
    simulator.data.to_csv("data/simulated_data_200.csv", index=False)
