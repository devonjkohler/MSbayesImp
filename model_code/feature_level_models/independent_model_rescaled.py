...
import builtins
import os
sys.stdout = open("../stdout_scaled.txt", "w", buffering=1)
def print(text):
    builtins.print(text)
    os.fsync(sys.stdout)
...
def feature_level_model(data, missing, priors):
    ...
    beta0 = numpyro.sample("beta0", numpyro.distributions.LogNormal(-5, .1))
    ...
    run_mu_list = numpyro.sample("mu", numpyro.distributions.Normal(priors["run_effect"], .5))
    ...
    sigma_list = numpyro.sample("error", numpyro.distributions.Exponential(5.).expand([len(priors["run_effect"])]))
...
def main():
    ...
    # save_folder = r"../../model_results/Choi2017/Choi2017_scaled_"
    save_folder = r"/home/kohler.d/MSbayesImp/model_results/Choi2017_"
    ...
    # input_data.loc[:, "Intensity"] = preprocessing.scale(input_data.loc[:, "Intensity"].values.reshape(-1, 1))
    scaler = StandardScaler().fit(input_data.loc[:, "Intensity"].values.reshape(-1, 1))
    input_data.loc[:, "Intensity"] = scaler.transform(input_data.loc[:, "Intensity"].values.reshape(-1, 1))
    # sample_proteins = np.random.choice(input_data["Protein"].unique(), 200, replace=False)
    # sample_proteins = input_data["Protein"].unique()[:1500]
