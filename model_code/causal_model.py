...
def two_nodes_run_level(data, missing, network, runs=100, features=5, learn_priors_from_data=True,
                        priors=None):
    ...
    if learn_priors_from_data:
        ...
        for i in range(len(data)):
            ...
            run_mu_list.append(numpyro.sample("mu_{}".format(i),
                                              numpyro.distributions.Normal(priors["run_effect_{}".format(i)], 5.)))
...
def causal_model(posterior_samples, missing, network, runs=100, features=3):

    nodes = list(nx.topological_sort(network))

    for i in nodes:

        beta0_temp = numpyro.sample("causal_int_{}".format(i), numpyro.distributions.Normal(10., 10.))
        ...
        if len(in_edges) > 0:
            for e in in_edges:
                causal_param = numpyro.sample("bC_{0}_{1}".format(i, e[0]),
                                              numpyro.distributions.Normal(-.5, .3))
                causal_effect += causal_param * (posterior_samples[e[0]])
        else:
            causal_effect = jnp.asarray([0 for _ in range(runs)])


        imp = numpyro.sample(
            "run_imp_{}".format(i), numpyro.distributions.Normal(
                beta0_temp + causal_effect[missing[i]],
                sigma_temp).mask(False)
        )

        observed = jnp.asarray(posterior_samples[i]).at[missing[i]].set(imp)
        numpyro.sample("obs_{}".format(i), numpyro.distributions.Normal(beta0_temp + causal_effect, sigma_temp),
                       obs=observed)
...
class CausalModel:
    ...
    def train(self, data, num_iters, initial_lr, gamma):
        ...
        mcmc = MCMC(NUTS(two_nodes_run_level), num_warmup=5000, num_samples=5000)
        ...
        run_mcmc = MCMC(NUTS(causal_model), num_warmup=2000, num_samples=2000)
        run_mcmc.run(random.PRNGKey(69), posterior_samples, missing, self.causal_network)
